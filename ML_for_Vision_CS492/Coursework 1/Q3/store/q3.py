...
def low_dimension_PCA_matrix(data, label, M_pca):
    low_D_dimension_eigenvectors, mean_face, decrease_order_pair_eigenvalue_eigenvector = low_dimension_PCA(data, label)
...
def incremental_PCA(M_pca):
    ...
    a1 = time.time()
    print("merge time: ", a1-a0)

    t1_incremental_PCA = time.time()
    print("Training time for incremental PCA: ", t1_incremental_PCA - t0_incremental_PCA)
...
def batch_PCA(M_pca):
    ...
    t1_batch = time.time()
    print("Training time for batch PCA: ", t1_batch - t0_batch)
...
def PCA_only_first_subset(M_pca):
    t0_PCA_only_first = time.time()
    mean_face_sub1, S1, W_pca_sub1 = PCA_matrix(sub_trainX_1, sub_train_l_1, M_pca)
    t1_PCA_only_first = time.time()
    print("Training time for PCA only on first subset: ", t1_PCA_only_first - t0_PCA_only_first)
...
def reconstruction_error_for_3_model(M_pca):
    ...
    print("w_pca_merge1234.shape = ", w_pca_merge1234.shape)
    incremental_pca_error = compute_mean_reconstruct_error(mean_face_merge1234, w_pca_merge1234, test_X)
    print("average reconstruction error of incremental PCA: ", incremental_pca_error)

    mean_face_batch, w_pca_batch = batch_PCA(M_pca)
    print("w_pca_batch.shape = ", w_pca_batch.shape)
    batch_error = compute_mean_reconstruct_error(mean_face_batch, w_pca_batch, test_X)
    print("average reconstruction error for batch PCA: ", batch_error)
    ...
    print("average reconstruction error for only first training: ", only_first_error)

reconstruction_error_for_3_model(200)
