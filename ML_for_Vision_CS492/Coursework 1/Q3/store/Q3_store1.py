...
#mean_face_merge1234, w_pca_merge1234 = incremental_PCA()
...
def reconstruct_single_img(img, mean_face_, w_pca, h = 46, w = 56):
    # w_pca.shape = (M_pca, D)
    copy_img = img.copy()
    xn = copy_img.flatten()
    ...
    tmp_reconstructed_face_i = mean_face_
...
def compute_mean_reconstruct_error(mean_face_, w_pca_):
    list_reconstruct_error_for_train_x = []
    for i in range(train_X.shape[0]):
        img_ = train_X[i]
        reconstruct_ = reconstruct_single_img(img_, mean_face_, w_pca_)
        reconstruct_error = np.linalg.norm(img_ - reconstruct_)
        list_reconstruct_error_for_train_x.append(reconstruct_error)
    list_reconstruct_error_for_train_x = np.array(list_reconstruct_error_for_train_x)
    mean_reconstruct_error_for_train_x = np.mean(list_reconstruct_error_for_train_x)
    return mean_reconstruct_error_for_train_x

def reconstruction_error_for_3_model():
    mean_face_merge1234, w_pca_merge1234 = incremental_PCA(70)
    incremental_pca_error = compute_mean_reconstruct_error(mean_face_merge1234, w_pca_merge1234)

    mean_face_batch, w_pca_batch = batch_PCA(70)
    batch_error = compute_mean_reconstruct_error(mean_face_batch, w_pca_batch)

    mean_face_sub1, w_pca_sub1 = PCA_only_first_subset(70)
    only_first_error = compute_mean_reconstruct_error(mean_face_sub1, w_pca_sub1)
