            ...
            if isinstance(node, ast.If) or isinstance(node, ast.For) or isinstance(node, ast.While) or isinstance(node,
                ...
                for handler_ in list_handlers:
                    ...
                    if (lineno_handler, col_offset_handler) not in B.dict_if.keys():
                        B.dict_if[(lineno_handler, col_offset_handler)] = {'executed': 0, 'try_line': try_lineno,
                                                                           'verbose': verbose_}
            ast.NodeVisitor.generic_visit(self, node)
    ...
    if args.verbose:
        ...
        for coordinate in B.dict_if.keys():
            if 'executed' in B.dict_if[coordinate].keys():
                ...
                full_verbose_branch = "Line %d: " % line_num + code_verbose
                sort_staff = {'code_verbose': code_verbose, 'line_num': line_num}
                list_branch_if.append((coordinate[0], coordinate[1], B.dict_if[coordinate]['true'],
                                       B.dict_if[coordinate]['false'], sort_staff))
        #################################
        """branch_printed_verbose = []
        for branch_ in list_branch_if:
            if len(branch_) == 5:
                if branch_[2] > 0:
                    branch_printed_verbose.append( branch_[-1] + " ==> True" )
                if branch_[3] > 0:
                    branch_printed_verbose.append( branch_[-1] + " ==> False" )
            elif len(branch_) == 4:  # ast.Try
                if branch_[2] > 0:
                    branch_printed_verbose.append(branch_[-1])
        branch_printed_verbose = sorted(branch_printed_verbose)
        # print
        for branch_printed_ in branch_printed_verbose:
        ...
        for branch_ in list_branch_if:
            ...
            if len(branch_) == 5:  # not ast.Try
                ...
                if branch_[3] > 0:
                    sort_string = code_verbose + " ==> False"
        ...
        for condition_ in list_condition_:
            ...
            if condition_[2] > 0:
                sort_string = code_verbose + " ==> True"
                printed_condition_verbose = "Line %d: " % line_num + sort_string
