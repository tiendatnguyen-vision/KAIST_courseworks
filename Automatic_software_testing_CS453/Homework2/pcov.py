...
if __name__ == '__main__':
	...
	class Initial_visit_branch(ast.NodeVisitor):
		def generic_visit(self, node):
			if isinstance(node, ast.If) or isinstance(node, ast.For) or isinstance(node, ast.While) or isinstance(node,ast.AsyncFor) or isinstance(node, ast.IfExp):
				lineno_ = node.lineno
				col_offset_ = node.col_offset
				# find verbose
				...
				for handler_ in list_handlers:
					...
					if (lineno_handler, col_offset_handler) not in B.dict_if.keys():
						B.dict_if[(lineno_handler, col_offset_handler)] = {'executed': 0,'try_line': try_lineno ,'verbose': verbose_}
			ast.NodeVisitor.generic_visit(self, node)
					...
					if len(predicate_.ops) > 1:
						...
						if (start_r, start_c, end_r, end_c) not in C.dict_con.keys():
							...
							for i in range(len(predicate_.ops)):
								divided_pred = ast.Compare(left=list_operands[i], ops=[predicate_.ops[i]], comparators=[list_operands[i + 1]])
								divided_pred_node = ast.Module(body=[ast.Expr(value=divided_pred)], type_ignores=[])
								verbose_ = ast.unparse(divided_pred_node)
								C.dict_con[(start_r, start_c, end_r, end_c)][str(i)] = {'verbose': verbose_, 'true': 0, 'false': 0}
					else:
						start_r, start_c, end_r, end_c = predicate_.lineno, predicate_.col_offset, predicate_.end_lineno, predicate_.end_col_offset
						verbose_ = merge_line(lines, start_r, start_c, end_r, end_c)
						if (start_r, start_c, end_r, end_c) not in C.dict_con.keys():
							C.dict_con[(start_r, start_c, end_r, end_c)] = {'conjunction': False, 'verbose': verbose_, 'true': 0, 'false': 0}
				else:
					start_r, start_c, end_r, end_c = predicate_.lineno, predicate_.col_offset, predicate_.end_lineno, predicate_.end_col_offset
					verbose_ = merge_line(lines, start_r, start_c, end_r, end_c)
					if (start_r, start_c, end_r, end_c) not in C.dict_con.keys():
		...
		def rewrite_atomic_predicate(self, predicate_):
			...
			if isinstance(predicate_, ast.Compare):
				if len(predicate_.ops) > 1:
					...
					for ops_ in ori_ops:
						lambda_ops = ast.Lambda(args=default_ops_args, body=ast.Compare(left=ast.Name(id='x'), ops=[ops_], comparators=[ast.Name(id='y')]))
	...
	class Rewrite_tree(ast.NodeTransformer):
		...
		def track_branch(self, node):
			...
			if isinstance(node, ast.If):
				...
				new_body = [true_branch_] + node.body
				new_orelse = [false_branch_] + node.orelse
				new_node = ast.For(target=node.target, iter=node.iter, body=new_body, orelse=new_orelse, type_comment=node.type_comment)
				ast.copy_location(new_node, node)  # the keypoint to overcome error 'For' object has no attribute 'lineno'
			...
			elif isinstance(node,ast.Try):  # B.dict_if[(lineno_handler, col_offset_handler)] = {'executed': 0, 'verbose': verbose_}
				...
				for old_handler in list_old_handlers:
					...
					executed_check = ast.parse("B.dict_if[(%d,%d)]['executed'] +=1" % (lineno_handler, col_offset_handler)).body[0]
				...
				new_node = ast.Try(body=node.body, handlers=list_new_handlers, orelse=node.orelse, finalbody=node.finalbody)
				ast.copy_location(new_node, node)
	...
	exec(ast.unparse(root), {"S": S, "B": B, "help_Ifexp": help_Ifexp, "C": C, "help_visit_Compare": help_visit_Compare, "help_modify_condition": help_modify_condition})
	...
	print("Statement Coverage: " + "{:.2f}%".format(percent_stmt_covered) + " (%d / %d)" % (num_stmt_covered, total_num_stmt))
	total_num_branch = 0
	...
	print("Condition Coverage: " + "{:.2f}%".format(percent_condition_covered) + " (%d / %d)" % (num_condition_covered, total_num_condition))
	...
	if args.verbose:
		...
		for coordinate in B.dict_if.keys():
			if 'executed' in B.dict_if[coordinate].keys():
				code_verbose = B.dict_if[coordinate]['verbose']
				line_num = B.dict_if[coordinate]["try_line"]
				full_verbose_branch = "Line %d: " %line_num + code_verbose
				sort_staff = {'code_verbose': code_verbose, 'line_num': line_num}
				list_branch_if.append((coordinate[0], coordinate[1], B.dict_if[coordinate]['executed'],  sort_staff))
			else:
				code_verbose = B.dict_if[coordinate]['verbose']
				line_num = coordinate[0]
				full_verbose_branch = "Line %d: " %line_num + code_verbose
				sort_staff = {'code_verbose': code_verbose, 'line_num': line_num}
				list_branch_if.append((coordinate[0], coordinate[1], B.dict_if[coordinate]['true'], B.dict_if[coordinate]['false'], sort_staff))

		dict_branch_verbose = {}
		for branch_ in list_branch_if:
			line_num = branch_[-1]['line_num']
			dict_branch_verbose[line_num] = []
		for branch_ in list_branch_if:
			line_num = branch_[-1]['line_num']
			code_verbose = branch_[-1]['code_verbose']  # (x == 10 and y == 5)

			if len(branch_) == 5: # not ast.Try
				if branch_[2] > 0:
					sort_string = code_verbose
					final_printed_verbose = "Line %d: " %line_num + code_verbose + " ==> True"   # Line 6: (x == 10 and y == 5) ==> True
					dict_branch_verbose[line_num].append((sort_string, final_printed_verbose))
				if branch_[3] > 0:
					sort_string = code_verbose
					final_printed_verbose = "Line %d: " %line_num + code_verbose + " ==> False"
					dict_branch_verbose[line_num].append((sort_string, final_printed_verbose))
			elif len(branch_) == 4:  # ast.Try
				if branch_[2] > 0:
					sort_string = code_verbose
					final_printed_verbose = "Line %d: " %line_num + code_verbose
					dict_branch_verbose[line_num].append((sort_string, final_printed_verbose))
		for line_num_ in dict_branch_verbose.keys():
			list_pairs = dict_branch_verbose[line_num_]
			dict_branch_verbose[line_num_] = sorted(list_pairs, key= lambda x: x[0])
		tmp_list = []
		for line_num_ in dict_branch_verbose.keys():
			tmp_list.append((line_num_, dict_branch_verbose[line_num_]))
		tmp_list = sorted(tmp_list, key=lambda x:x[0])
		branch_printed_verbose_list = []
		for pair_linenum_remain in tmp_list:
			list_for_extend = [ele[1] for ele in pair_linenum_remain[1]]
			branch_printed_verbose_list.extend(list_for_extend)

		# print
		for branch_printed_ in branch_printed_verbose_list:
			print(branch_printed_)
		...
		for coordinate in C.dict_con.keys():
			if C.dict_con[coordinate]['conjunction'] == False:
				list_condition_.append((coordinate[0], coordinate[1], C.dict_con[coordinate]['true'],
										C.dict_con[coordinate]['false'], C.dict_con[coordinate]['verbose']))
			else:
				for str_i in C.dict_con[coordinate].keys():
					if str_i != 'conjunction':
						list_condition_.append((coordinate[0], coordinate[1], C.dict_con[coordinate][str_i]['true'],
												C.dict_con[coordinate][str_i]['false'],
												C.dict_con[coordinate][str_i]['verbose']))

		dict_verbose_condition = {}
		for condition_ in list_condition_:
			line_num = condition_[0]
			dict_verbose_condition[line_num] = []
		for condition_ in list_condition_:
			line_num = condition_[0]
			code_verbose = condition_[-1]
			if condition_[2] > 0:
				sort_string = code_verbose
				printed_condition_verbose = "Line %d: " % line_num + code_verbose + " ==> True"
				dict_verbose_condition[line_num].append((sort_string, printed_condition_verbose))
			if condition_[3] > 0:
				sort_string = code_verbose
				printed_condition_verbose = "Line %d: " % line_num + code_verbose + " ==> False"
				dict_verbose_condition[line_num].append((sort_string, printed_condition_verbose))
		for line_num_ in dict_verbose_condition.keys():
			list_pairs = dict_verbose_condition[line_num_]
			dict_verbose_condition[line_num_] = sorted(list_pairs, key=lambda x:x[0])
		con_tmp_list = []
		for line_num_ in dict_verbose_condition.keys():
			con_tmp_list.append( (line_num_, dict_verbose_condition[line_num_]) )
		con_tmp_list = sorted(con_tmp_list, key=lambda x:x[0])
		list_verbose_condition = []
		for pair_linenum_list in con_tmp_list:
			list_for_extend = [ele[1] for ele in pair_linenum_list[1]]
			list_verbose_condition.extend(list_for_extend)
		for printed_condition_ in list_verbose_condition:
			print(printed_condition_)
